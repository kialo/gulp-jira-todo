...
JiraTodo.prototype.processTODOs = function (todos, callback) {
    ...
    this.getJiraStatusForIssues(_.map(allIssues, 'key'), function (err, statuses) {
...
JiraTodo.prototype.getJiraStatusForIssues = function (issueKeys, callback) {
    ...
    async.eachLimit(_.uniq(issueKeys), 3, function (issueKey, cb) {
        ...
        fetch(url, {
            ...
            return cb(
                util.newPluginError('Error retrieving status for issue "' + issueKey + '": "' + err.toString() + '".')
            );
            ...
            return cb(
                util.newPluginError('Error parsing JSON response for issue "' + issueKey + '": "' + e.message + '".')
            );
        }).then(function (data) {
            if (!data) {
                return;
